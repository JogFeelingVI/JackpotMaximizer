...
# @Last Modified time: 2024-06-02 23:24:26
...
def loadLine(path: pathlib.Path):
    ...
    try:
        ...
        end_time = time.perf_counter()
        print(
            f'This running time is {g(f"{end_time-start_time-3:.4f}")} seconds. {insert_test.__len__()}'
        )
...
def tongji(path: pathlib.Path):
    ...
    try:
        ...
        end_time = time.perf_counter()
        print(
            f'This running time is {g(f"{end_time-start_time:.4f}")} seconds. {insert_test.__len__()}'
        )
    except Exception as e:
        print(f"ERR: {e}")
        with open("error.log", "a") as file:
...
def monitor(tasks: list, finished_event: threading.Event):
    while True:
        if not tasks and finished_event.is_set():
            print(
                "The workers have gone off work and the monitors are about to take a break."
            )
            break
        else:
            ...
            print(
                f"The worker is in good condition and has completed `{r(size)}`, working effectively. Last Modified {r(last_time)}. Tasks {tasks.__len__()}"
            )
        time.sleep(30)
    ...
    match sys.argv:
        ...
        case [_, "load", path]:
            ...
            if (p := pathlib.Path(path)).exists():
                ...
                print(
                    f"Use `load` to start the program, but the `{path}` file does not exist."
                )
        case [_, "tongji", path]:
            print(f'{g(f"Use `Load {path}` to execute the program")}')
            if (p := pathlib.Path(path)).exists():
        ...
        case [_, "load"]:
            print(
                f'{r("To start a program using `load`, the specified file must be provided.")}'
            )
            print(
                f'{r("For example `poetry run python3 ./jpm_insight.py load ./outing_id_r_b.log`")}'
            )
        case _:
